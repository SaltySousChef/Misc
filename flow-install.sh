...
sudo apt update
sudo apt install build-essential libssl-dev apt-transport-https ca-certificates gnupg lsb-release -y
...
# generate keys
mkdir ./bootstrap
./boot-tools/bootstrap key --address "$SUBDOMAIN.$DOMAIN:3569" --role $NODE_ROLE -o ./bootstrap >> ./bootstrap/boostrap-key-log.txt
